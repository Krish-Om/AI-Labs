...
# Global variables for Greedy solution
solution_path = []
current_step = 0
solving = False
auto_play = False
...
def gameLoop():
    ...
    print("\n 8-Puzzle with Greedy Best-First Search Solver")
    print("Controls:")
    print("  G: Run Greedy Best-First search")
    ...
    if event.type == pygame.QUIT:
        ...
        if event.key == pygame.K_r:
            ...
            if solution_path:
                ...
                print(
                    "No solution loaded! Press 'G' to run Greedy Best-First Search first."
                )
...
def runGreedySearch(puzzle):
    """Run Greedy Best-First Search algorithm and store solution"""
    global solution_path, current_step, solving, original_puzzle_state, auto_play

    print("\n" + "=" * 50)
    print(
        f"Running Greedy Best-First Search with {heuristic_type.title()} heuristic..."
    )
    print("=" * 50)
    ...
    try:
        solver = GreedyBestFirstSolver(puzzle, heuristic_type)
        ...
        if success:
            ...
            print(
                f"    -Note: Greedy may not find optimal solution (uses only heuristic)"
            )
        ...
        print(f" Greedy Best-First Search Error: {e}")
        solution_path = []
