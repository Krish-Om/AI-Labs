...
# Global variables for DLS solution
solution_path = []
current_step = 0
solving = False
auto_play = False
...
current_depth_limit = 20  # Default depth limit
...
def gameLoop():
    ...
    print("\n 8-Puzzle with Depth Limited Search (DLS) Solver")
    print("Controls:")
    print("  L: Run DLS search")
    print("  +/-: Increase/Decrease depth limit")
    print("  SPACE: Show next move in solution")
    print("  A: Auto play the moves")
    print("  R: Shuffle puzzle")
    print("  ESC: Quit")
    print("  Click: Manual move")
    print(f"  Current depth limit: {current_depth_limit}")
    print()
...
def handleInput(event, puzzle):
    global solution_path, current_step, solving, auto_play, original_puzzle_state, current_depth_limit
    ...
    if event.type == pygame.QUIT:
        ...
        if event.key == pygame.K_r:
            ...
            # Increase depth limit (only if puzzle is not solved)
            if puzzle.checkWin():
                print(
                    "Cannot change depth limit - puzzle is already solved! Press 'R' to shuffle first."
                )
            else:
                current_depth_limit += 5
                print(f"Depth limit increased to: {current_depth_limit}")
        elif event.key == pygame.K_MINUS:
            # Decrease depth limit (only if puzzle is not solved)
            if puzzle.checkWin():
                print(
                    "Cannot change depth limit - puzzle is already solved! Press 'R' to shuffle first."
                )
            else:
                current_depth_limit = max(5, current_depth_limit - 5)
                print(f"Depth limit decreased to: {current_depth_limit}")
            ...
            if solution_path:
                ...
                print("No solution loaded! Press 'L' to run DLS first.")
...
def printPuzzleState(puzzle):
    ...
    if puzzle.checkWin():
        ...
        print("=" * 50)
        print("Controls:")
        print("  L: Run DLS search (disabled - already solved)")
        print("  +/-: Change depth limit (disabled - shuffle first)")
        print("  SPACE: Show next move in solution")
        print("  A: Auto play the moves")
        print("  R: Shuffle puzzle")
        print("  ESC: Quit")
...
def runDLS(puzzle):
    """Run DLS algorithm and store solution"""
    global solution_path, current_step, solving, original_puzzle_state, auto_play

    print("\n" + "=" * 50)
    print(f"Running DLS (Depth Limited Search) with limit: {current_depth_limit}")
    print("=" * 50)
    ...
    try:
        solver = DepthLimitedSolver(puzzle, current_depth_limit)
        ...
        if success:
            ...
            print(f"    -Depth limit used: {current_depth_limit}")
            print(f"    -Nodes explored: {solver.nodes_explored}")
            print(f"    -Max depth reached: {solver.max_frontier_size}")
            ...
            print(f" No solution found within depth limit of {current_depth_limit}!")
            print(f" Try increasing the depth limit with '+' key and search again.")
            solution_path = []
