...
import model
from uniform_cost_solver import UniformCostSolver

pygame.init()
BOARD_SIZE = 3

# Global variables for UCS solution
solution_path = []
current_step = 0
solving = False
auto_play = False
...
def gameLoop():
    ...
    print("\n 8-Puzzle with Uniform Cost Search (UCS) Solver")
    print("Controls:")
    print("  U: Run UCS search")
    ...
    if event.type == pygame.QUIT:
        ...
        if event.key == pygame.K_r:
            ...
            if solution_path:
                ...
                print("No solution loaded! Press 'U' to run UCS first.")
...
def printPuzzleState(puzzle):
    ...
    if puzzle.checkWin():
        ...
        print("=" * 50)
        print("Controls:")
        print("  U: Run UCS search (disabled - already solved)")
        print("  SPACE: Show next move in solution")
        print("  A: Auto play the moves")
        print("  R: Shuffle puzzle")
        print("  ESC: Quit")
...
def runUCS(puzzle):
    """Run UCS algorithm and store solution"""
    global solution_path, current_step, solving, original_puzzle_state, auto_play

    print("\n" + "=" * 50)
    print("Running UCS (Uniform Cost Search)...")
    print("=" * 50)
    ...
    try:
        ...
        if success:
            ...
            print(f"    -Note: UCS finds optimal solution (same as BFS for unit costs)")
